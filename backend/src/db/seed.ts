 ...
 *
 * Safe to run on every deploy. It never touches transactional data.
 */
...
export async function seedCore(): Promise<{ adminCreated: boolean; generatedPassword?: string }> {
  return db.transaction(async (tx) => {
    ...
    if (activeAdmins.length > 0) return { adminCreated: false };
    ...
    if (taken) throw new Error(`No active administrator exists, but username "${username}" is taken. Set SEED_ADMIN_USERNAME to a free username.`);
    ...
    return { adminCreated: true, generatedPassword };
  });
}
...
if (import.meta.main) {
  try {
    ...
    if (result.generatedPassword) {
      ...
      console.log(`\nAdministrator created: username "${env.SEED_ADMIN_USERNAME}", temporary password: ${result.generatedPassword}\nIt must be changed at first sign-in.\n`);
    }
    await closeDb();
    process.exit(0);
  } catch (err) {
